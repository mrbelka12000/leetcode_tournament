...
func (t *tournament) Update(ctx context.Context, obj models.TournamentCU, id int64) error {
	...
	_, err := Exec(ctx, t.db, queryUpdate+querySet+queryWhere, updateValues...)
	if err != nil {
...
`
	...
	row := QueryRow(ctx, t.db, querySelect+queryFrom+queryWhere, filterValues...)
	err := row.Scan(
		&tr.ID,
		&tr.UsrID,
		&tr.StartTime,
...
`
	...
	var tCount int64
	if pars.Limit > 0 {
		err := QueryRow(ctx, t.db, `select count(*)`+queryFrom+queryWhere, filterValues...).Scan(&tCount)
		if err != nil {
			return nil, 0, err
		}
		if pars.OnlyCount {
	...
	rows, err := Query(ctx, t.db, querySelect+queryFrom+queryWhere+queryOrderBy+queryOffset+queryLimit, filterValues...)
	if err != nil {
